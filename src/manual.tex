...
The interactive demo consists of textured cubes in a procedurally generated 3-dimensional world.  The texturing of the cubes is handled through UV Mapping.  All blocks textures are defined in a spritesheet that contains the blocks textures stored as several smaller graphics in a tiled grid arrangement.  The spritesheet uses a single value to index each tile in the grid arrangement.  The value is computed by determining the current ($x$, $y$) position the cube texture, then creating a ratio from the total size of the spritesheet.
\vskip 2.5mm\noindent
The texturing of the blocks can be toggled by the key \keys{3}.  The texture mapping is visible in Appendix \ref{appendix:world}.  The world with texturing toggled off is visible at Appendix \ref{appendix:shadows} and \ref{appendix:occlusion}.
...
The interactive demo includes dust particles that appear after the destruction of a block.  When the block is destroyed, a set of dust particles appear for a short duration, moving in a random direction and velocity away from the center of the destroyed block.  The dust particles are scaled down based on a factor of time, disappearing after a short period.  The movement code requires that the particles will always move away from the center of the destroyed cube.  The particles orient themselves based on Billboarding, ensuring that they face towards the camera direction.  In graphics, a billboard is a textured flat object, usually a polygon, which faces the camera.  Billboarding is a operation to orient an object to always faces the camera direction.  In the case of the particles, they are a cube to keep consistent with the Minecraft's style.  The direction changes constantly so that the particles will always faces toward the camera direction.  The method used with the particles requires the removal of the rotational component of the world-view matrix.  The inner \texttt{3x3} matrix is set to the identity matrix, then the scalar component is reapplied.  The final result is a dust particle that always faces towards the player.
\vskip 2.5mm\noindent
