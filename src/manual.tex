...
\begin{itemize}
	...
	\item \keys{J}/\keys{K} - Increases/Decreeases thge frequency value used in the Octave Perlin Noise
	\item \keys{C}/\keys{V} - Increases/Decreases the radius of chunks present in the scene.
	\item \keys{F}/\keys{G} - Shifts the current selected block either left or right.
	\item \keys{P} - Toggles the particle visibility type
	\item \keys{O} - Toggles the orientation component of the Cloud animation.
	\item \keys{L} - Toggles the currently targetted block (by Crosshairs) to become a Light Emitting block.
\end{itemize}
